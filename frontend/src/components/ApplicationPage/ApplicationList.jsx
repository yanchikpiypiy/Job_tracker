...
export default function ApplicationsList() {
    const { applications, deleteUserApplication, fetchApplications } = useContext(ApplicationsContext)
    ...
    // Calculate number of selected jobs
    const selectedCount = Object.values(jobTicked).filter(Boolean).length;
    ...
    async function handleBulkDelete() {
        const selectedJobIds = Object.keys(jobTicked).filter(id => jobTicked[id]);

        if (selectedJobIds.length === 0) return;

        // Confirm before deleting
        const confirmDelete = window.confirm(
            `Are you sure you want to delete ${selectedJobIds.length} selected application${selectedJobIds.length > 1 ? 's' : ''}?`
        );
        ...
        if (confirmDelete) {
            try {
                ...
                for (const jobId of selectedJobIds) {
                    await deleteUserApplication(jobId);
                }
                ...
                setJobTicked({});
                setTicked(Object.fromEntries(grouped.map(g => [g.status, false])));
            } catch (error) {
                ...
                <div className={styles.applicationMain}>
                    ...
                    {grouped.map(group => (
                        <div key={group.status}>
                            ...
                            >
                                <span
                                    className={`${styles.caret} ${collapsed[group.status] ? styles.caretDown : styles.caretUp
                                        }`}
                                />
